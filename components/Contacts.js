...
const Contacts = () => {
  ...
  const [user, setUser] = useState(null); // FIXED: Proper auth state
  ...
  const loadFromCache = useCallback(() => {
    const cached = getFromStorage(LOCAL_STORAGE_KEY);
    if (cached) {
      try {
  ...
  const saveToCache = useCallback((contacts) => {
    setToStorage(LOCAL_STORAGE_KEY, JSON.stringify(contacts));
    setToStorage(LAST_SYNC_KEY, new Date().toISOString());
  }, []);
  ...
  }, [user, loadFromCache, loadFromCloud, isOnline]);
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0a0a0a] p-6 md:p-10 font-sans transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-6 flex items-center justify-between text-xs text-zinc-400">
          <p>Last synced: {getFromStorage(LAST_SYNC_KEY) ? new Date(getFromStorage(LAST_SYNC_KEY)).toLocaleString() : 'Never'}</p>
