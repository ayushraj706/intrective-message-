...
const Contacts = () => {
  ...
  const [extractedContacts, setExtractedContacts] = useState([]); // Array of objects: {number, isWhatsApp}
  ...
  // 3. व्हाट्सएप वेरिफिकेशन (Simulation)
  const verifyNumbers = (numbers) => {
    return numbers.map(num => ({
      number: num,
      // अभी के लिए हम +91 वाले नंबर्स को वेरीफाइड मान रहे हैं (Testing के लिए)
      isWhatsApp: num.startsWith('+91') 
    }));
  };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      if (phoneIndex === -1) {
        alert("फोन नंबर वाला कॉलम नहीं मिला!");
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (phone) {
          ...
          if (cleanPhone.length >= 10) numbersList.push('+' + cleanPhone);
        }
      }

      const verifiedContacts = verifyNumbers([...new Set(numbersList)]);
      const finalContacts = [...extractedContacts];
      
      // पुराने नंबर्स में नए जोड़ना (Duplicate चेक के साथ)
      verifiedContacts.forEach(newC => {
        if (!finalContacts.some(oldC => oldC.number === newC.number)) {
          finalContacts.push(newC);
        }
      });

      setExtractedContacts(finalContacts);
      await syncToCloud(finalContacts);
      setIsUploading(false);
      alert(`${verifiedContacts.length} नंबर्स प्रोसेस हो गए! ✅`);
    };
  ...
  const filteredContacts = extractedContacts.filter(c => c.number.includes(searchTerm));

  return (
    <div className="min-h-screen bg-white dark:bg-[#050505] p-6 md:p-10 font-sans">
      <div className="max-w-6xl mx-auto">
        
        {/* Header Section with WhatsApp Style Plus Button */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <h1 className="text-5xl font-black tracking-tighter dark:text-white italic uppercase">BaseKey Cloud</h1>
            <p className="text-zinc-500 text-sm mt-2 font-medium">WhatsApp Verified Lists.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="bg-white dark:bg-zinc-900/50 p-8 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm h-fit">
            ...
            <p className="text-zinc-500 text-xs font-bold uppercase tracking-[0.2em]">Verified Contacts</p>
            ...
            <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/80 backdrop-blur-md">
              <div className="relative w-full">
                ...
                <input 
                  ...
                  className="w-full bg-zinc-100 dark:bg-zinc-800/50 rounded-2xl py-3.5 pl-12 pr-4 text-sm outline-none dark:text-white ring-blue-500/10 focus:ring-2 transition-all" 
                />
              </div>
            </div>
              ...
              {filteredContacts.length > 0 ? filteredContacts.map((contact, i) => (
                <div key={i} className="group p-5 flex items-center justify-between hover:bg-zinc-50 dark:hover:bg-blue-600/5 transition-all">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button className="flex items-center gap-2 bg-blue-600/10 text-blue-600 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-blue-600 hover:text-white transition-all">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-20 opacity-30">
                  ...
                  <p className="text-zinc-400 font-bold uppercase tracking-[0.3em] text-[10px]">Empty Cloud List</p>
                </div>
              )}
            </div>
          </div>
