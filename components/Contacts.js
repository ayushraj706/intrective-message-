...
import { Upload, Users, Search, Smartphone, FileText, Loader2, Plus, ArrowRight } from 'lucide-react';
...
const Contacts = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      // तुम्हारे द्वारा बनाई गई API को कॉल करना
      const res = await axios.post('/api/extract-contacts', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      
      setExtractedContacts(res.data.numbers);
      alert(`सफलता! ${res.data.numbers.length} नंबर्स मिल गए हैं। 🚀`);
    } catch (err) {
      console.error(err);
      alert("Extraction फेल हो गया! API चेक करें।");
    }
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#050505] p-6 md:p-10 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 animate-in fade-in duration-700">
          <div>
            <h1 className="text-5xl font-black tracking-tighter dark:text-white italic">Contacts</h1>
            <p className="text-zinc-500 text-sm mt-2 font-medium">Extract bulk WhatsApp numbers from any document.</p>
          </div>
          ...
          <label className="group flex items-center gap-3 bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-bold cursor-pointer transition-all shadow-xl shadow-blue-600/20 active:scale-95">
            ...
            <span>{isUploading ? 'Scanning File...' : 'Upload List'}</span>
          ...
          <div className="bg-white dark:bg-zinc-900/50 p-8 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm h-fit">
            ...
            <div className="mt-8 pt-8 border-t border-zinc-100 dark:border-zinc-800">
              <p className="text-zinc-400 text-xs leading-relaxed">
                जैसे ही आप इन नंबर्स को मैसेज करेंगे, व्हाट्सएप उनका **असली नाम** अपने आप हमारे इनबॉक्स में अपडेट कर देगा।
              </p>
            </div>
          </div>
              ...
              {filteredContacts.length > 0 ? filteredContacts.map((num, i) => (
                <div key={i} className="group p-5 flex items-center justify-between hover:bg-zinc-50 dark:hover:bg-blue-600/5 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-bold text-sm dark:text-zinc-200 tracking-tight">{num}</span>
                  </div>
