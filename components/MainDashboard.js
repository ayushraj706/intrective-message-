...
import { auth } from '../firebase'; // Auth import karo
...
import InstagramSetup from './setup/InstagramSetup'; // Import naya component

const MainDashboard = () => {
  const [setupMode, setSetupMode] = useState(null);
  
  // Current User ID nikalna
  ...
  // Logic to return setup pages with userId
  if (setupMode === 'whatsapp') return <WhatsAppSetup userId={currentUserId} onBack={() => setSetupMode(null)} />;
  if (setupMode === 'facebook') return <FacebookSetup userId={currentUserId} onBack={() => setSetupMode(null)} />;
  if (setupMode === 'instagram') return <InstagramSetup userId={currentUserId} onBack={() => setSetupMode(null)} />;
  if (setupMode === 'tg-api') return <TelegramAPISetup userId={currentUserId} onBack={() => setSetupMode(null)} />;
  if (setupMode === 'tg-bot') return <TelegramBotSetup userId={currentUserId} onBack={() => setSetupMode(null)} />;

  return (
    <div className="p-12 bg-zinc-50 dark:bg-[#080808] h-screen overflow-y-auto transition-colors duration-300">
      <div className="mb-12">
        <h1 className="text-4xl font-black tracking-tighter italic uppercase text-zinc-900 dark:text-white">BaseKey <span className="text-blue-500">Inboxes</span></h1>
        <p className="text-zinc-500 dark:text-gray-500 mt-2 text-lg font-medium tracking-tight">Select a node to activate automation neural link.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl">
        {channels.map((ch) => (
          ...
          >
            ...
            <h3 className="text-2xl font-black mb-3 text-zinc-900 dark:text-white uppercase tracking-tighter italic group-hover:text-blue-500 transition-colors">{ch.name}</h3>
