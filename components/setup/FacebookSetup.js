...
import { Facebook, Zap, Copy, RefreshCw, CheckCircle2, ArrowLeft } from 'lucide-react';
import { db } from '../../firebase';
import { doc, getDoc, setDoc } from 'firebase/firestore';

export default function FacebookSetup({ userId, onBack }) {
  const [loading, setLoading] = useState(false);
  const [config, setConfig] = useState({
    token: '',
    // Vercel URL Logic
    webhook: `https://intrective-message-vercel.app/api/messenger-webhook/${userId}`,
    status: 'idle'
  });
  ...
  const generateNode = async () => {
    setLoading(true);
    const newToken = 'BK_FB_' + Math.random().toString(36).substring(7).toUpperCase();
    try {
  ...
  return (
    <div className="p-12 bg-[#080808] min-h-screen text-white">
      ...
      <div className="max-w-2xl bg-[#111] p-10 rounded-[3rem] border border-white/5 shadow-2xl">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase italic tracking-tighter">Messenger <span className="text-blue-500">Node</span></h2>
              <p className="text-[10px] text-zinc-500 font-mono">Status: {config.status.toUpperCase()}</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="p-5 bg-black/50 border border-white/5 rounded-2xl flex justify-between items-center group">
            <div className="overflow-hidden">
              ...
              <p className="text-xs text-blue-400 font-mono truncate">{config.webhook}</p>
            </div>
        ...
        <button onClick={generateNode} disabled={loading} className="mt-8 w-full py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase text-xs tracking-[0.2em] transition-all flex items-center justify-center gap-3">
          {loading ? <RefreshCw className="animate-spin" /> : <Zap size={18} />} Deploy Meta Webhook
        </button>
      </div>
    </div>
  );
