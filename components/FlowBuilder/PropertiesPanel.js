...
const PropertiesPanel = ({ selectedNode, onUpdate, onDelete, onClose }) => {
  ...
  // typing handler jo seedhe id ke basis par sync karta hai
      ...
      <div className="space-y-6">
        {blocks.map((block, idx) => (
          <div key={block.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 relative group transition-all hover:border-indigo-100">
            {/* Individually delete a block */}
            <button 
              onClick={() => onUpdate(selectedNode.id, blocks.filter(b => b.id !== block.id))}
              className="absolute -top-2 -right-2 bg-white text-slate-300 hover:text-red-500 shadow-md border border-slate-100 p-1.5 rounded-full opacity-100 transition-all z-10"
            >
              <Trash2 size={12} />
            </button>
            ...
            {block.type === 'text' ? (
              <div className="space-y-2 text-left">
                ...
                <textarea 
                  value={block.content || ''} // Fallback to empty string for backspace fix
                  onChange={(e) => handleFieldUpdate(block.id, 'content', e.target.value)}
                  placeholder="Type your message..."
                  className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs outline-none focus:border-indigo-500 min-h-[100px] resize-none transition-all"
                />
              </div>
            ) : (
              <div className="space-y-3 text-left">
                ...
                <input 
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs outline-none focus:border-indigo-500"
                />
                {block.subType === 'url' && (
                  <input 
                    type="text" 
                    ...
                    className="w-full bg-slate-100 border border-slate-200 rounded-xl p-3 text-[10px] font-mono outline-none text-indigo-600 focus:border-indigo-500"
                  />
                )}
              </div>
            )}
        ...
        <div className="pt-4 border-t border-slate-100 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => addBlock('button', 'reply')} className="flex items-center justify-center gap-2 p-3 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-bold hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
              <Type size={14} /> + Quick Reply
            </button>
            <button onClick={() => addBlock('button', 'url')} className="flex items-center justify-center gap-2 p-3 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-bold hover:bg-slate-800 hover:text-white transition-all shadow-sm">
