...
const Inbox = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      try {
        await axios.post('/api/send-media', { userId, to: selectedRoom, mediaUrl, mediaType });
      ...
      <div className={`w-full md:w-80 border-r border-zinc-100 dark:border-zinc-800 flex flex-col bg-white dark:bg-[#0a0a0a] transition-all ${selectedRoom ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-20">
          {rooms.length > 0 ? rooms.map(num => (
            <div key={num} onClick={() => setSelectedRoom(num)} className={`p-4 mb-1 rounded-2xl flex items-center gap-4 cursor-pointer transition-all ${selectedRoom === num ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-zinc-50 dark:hover:bg-zinc-900 text-zinc-900 dark:text-zinc-400'}`}>
          ...
          <>
            ...
            <div className="p-4 bg-white dark:bg-black border-t border-zinc-100 dark:border-zinc-800 z-50">
              <form onSubmit={sendMessage} className="flex items-center gap-2 max-w-5xl mx-auto" autoComplete="off">
                ...
                <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Type your message..." className="flex-1 bg-zinc-100 dark:bg-zinc-900 rounded-2xl px-6 py-3.5 outline-none dark:text-white border border-transparent focus:border-blue-500/20 text-sm" />
