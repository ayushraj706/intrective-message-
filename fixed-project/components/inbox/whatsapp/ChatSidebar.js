      ...
      <div className="p-6">
        ...
        <div className="relative mt-4">
          ...
          <input 
            ...
            className="w-full bg-zinc-100 dark:bg-[#111] py-3 pl-12 pr-4 rounded-2xl text-xs outline-none focus:ring-1 ring-green-600/30 transition-all text-white"
          />
        </div>
      </div>
        ...
        {rooms.map((num, idx) => (
          <button 
            ...
            className={`w-full p-4 rounded-[2rem] flex items-center gap-4 transition-all duration-300 ${
              String(selectedRoom) === String(num) 
              ? 'bg-green-600 text-white shadow-xl shadow-green-600/20' 
              : 'hover:bg-zinc-100 dark:hover:bg-[#151515] dark:text-white'
            }`}
          >
            {/* SAFE SLICE: String mein convert karna zaroori hai */}
            <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${String(selectedRoom) === String(num) ? 'bg-white/20' : 'bg-green-600 text-white'}`}>
              {String(num).slice(-2)}
            </div>
            <div className="text-left flex-1 overflow-hidden">
              <p className="font-bold text-[14px] truncate">{num}</p>
              <p className={`text-[10px] opacity-60 ${String(selectedRoom) === String(num) ? 'text-white' : 'text-zinc-500'}`}>Secure Channel Active</p>
            </div>
          </button>
        ))}
      </div>
